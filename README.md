...
Запрос видеоверификации переводит агент в `waiting_verification` до обработки
примеров фото/видео. Ожидающие задания очищаются, текущие операции отменяются;
лайки, сообщения, recovery и `Skip` не отправляются. Перед стартовыми командами
проверяются последние 50 сообщений бота: обнаруженный запрос блокирует автоматику
и после перезапуска. Ошибка чтения истории также блокирует запуск обработки.

Автовозобновление пока заблокировано отсутствием подтверждённого формата успешного
ответа Leo: отправленный кружок, меню и новые медиа не считаются доказательством.
После ручного прохождения нужен фактический ответ бота для добавления безопасного
перехода из ожидания. История ограничена 50 сообщениями, постоянное состояние проверки
не хранится; запрос за пределами этого окна при новом запуске не обнаруживается.
...
7. Кэширование валидных решений (1000 записей); при старте решения
   восстанавливаются из локального audit-журнала, повторный LLM-запрос
   для известных анкет не выполняется
...
`action`, `reason`, `message`, модель, анкету, идентификаторы фото и промпт. `sent` означает успешный
